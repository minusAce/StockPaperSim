...
const parseTs = ts => { if (!ts) return null; const dt = new Date(ts); return Number.isNaN(dt.getTime()) ? null : dt }
...
const fmtTimeFull = dt => dt.toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit', second: '2-digit', hour12: true })
...
export default function App(){
  const [status,setStatus]=useState({}); const [now,setNow]=useState(new Date()); const [wsState,setWsState]=useState('DISCONNECTED'); const [lastUpdateAt,setLastUpdateAt]=useState(null); const [manualAi,setManualAi]=useState(false); const [agents,setAgents]=useState([]); const [portfolio,setPortfolio]=useState({positions:[]}); const [candidates,setCandidates]=useState([]); const [benchmark,setBenchmark]=useState({symbol:'S&P 500'}); const [decisions,setDecisions]=useState([]); const [orders,setOrders]=useState([]); const [performance,setPerformance]=useState({agents:[]}); const [logs,setLogs]=useState([]); const [killSwitchLatched,setKillSwitchLatched]=useState(false); const [activeDecision,setActiveDecision]=useState(null); const wsRef=useRef(null); const reconnectTimerRef=useRef(null); const wsPausedRef=useRef(false)
  ...
  const refresh = async () => {
    try {
      ...
      setAgents(a);setPortfolio(p);setCandidates(c);setBenchmark(b);setDecisions(d);setOrders(o);setPerformance(perf)
      return s
    } catch { return null }
  }

  const pushLog=(agent,msg,kind='')=>setLogs(x=>[{time:new Date().toLocaleTimeString(),agent,msg,kind},...x].slice(0,150))
  ...
  const connect=()=>{
    ...
    ws.onmessage=e=>{ const m=JSON.parse(e.data), d=m.data
      ...
      if(m.type==='decision'){setDecisions(x=>[d,...x].slice(0,80));pushLog('RISK',`${d.symbol} ${d.action} ${d.approved?'APPROVED':`REJECTED — ${prettyReason(d.rejection_reason||d.reason)}`}`,'risk')}
      if(m.type==='trade'){setOrders(x=>[d,...x].slice(0,80));pushLog('EXEC',`${d.action} ${d.qty} ${d.symbol} ${d.status||''}`,'trade')}
    ...
    ws.onclose=()=>{ setWsState('DISCONNECTED'); if (!wsPausedRef.current) reconnectTimerRef.current=window.setTimeout(connect,2000) }
    ws.onerror=()=>setWsState('ERROR')
  }

  const post=async(url,body)=>{try{await fetch(url,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(body||{})})}finally{await refresh(); connect()}}
  const runManualAi=async()=>{ setManualAi(true); try { const r=await fetch('/api/control/run-ai-test',{method:'POST'}); if(!r.ok){ const data=await r.json().catch(()=>({})); throw new Error(data.message||'Manual AI run failed') } await refresh() } catch(err){ pushLog('SYS', err.message||'Manual AI run failed','error') } finally { setManualAi(false) }}
  ...
  useEffect(()=>{
    ...
    return()=>{alive=false; clearInterval(c); if(reconnectTimerRef.current) clearTimeout(reconnectTimerRef.current); wsPausedRef.current=true; wsRef.current?.close()}
  },[])

  useEffect(()=>{
    if(autopilotOn || !wsPausedRef.current){ connect() }
    else {
  ...
  const benchmarkTicker = benchmark?.price ? [{symbol:benchmark.symbol, last_price:benchmark.price, change_pct:benchmark.day_change_pct, isBenchmark:true}] : []
  ...
  return <div className="app-shell">
    <header className="topbar">
      ...
      <div className="topbar-right">
        ...
        <div className={`metric topbar-right-metric realtime-status ${marketDataClass}`} title={lastUpdateAt ? `Last update ${lastUpdateAt.toLocaleTimeString()}` : 'Waiting for market data'} aria-label={`Market data: ${wsState}`}>
          <div className="realtime-main"><i/> <span>MARKET DATA</span></div>
          <div className="realtime-last">LAST UPDATE <b>{lastUpdateAt ? lastUpdateAt.toLocaleTimeString() : '—'}</b></div>
        </div>
        <div className="metric topbar-right-metric clock"><span>EAST COAST</span><time dateTime={now.toISOString()}>{now.toLocaleTimeString('en-US',{timeZone:'America/New_York',hour:'2-digit',minute:'2-digit',second:'2-digit',hour12:true})}</time></div>
      </div>
    </header>
    ...
    <div className="ticker-bar">
      <div className="ticker-viewport">
        <div className="ticker-track">
          {tickerLoop.map((c,i)=>{ const direction=movementClass(c.change_pct); return <div key={`${c.symbol}-${i}`} className={`ticker ${direction} ${c.isBenchmark?'benchmark-ticker':''}`}><b>{c.symbol}</b><small>{c.last_price?money(c.last_price):'—'}</small><span className={direction}>{c.change_pct==null?'—':`${c.change_pct >= 0 ? '+' : ''}${pct(c.change_pct)}`}</span></div> })}
        </div>
      </div>
    </div>
    ...
    <main className="floor-grid">
      ...
      <section className="panel market">
        ...
        <div className="panel-scroll monitor-table">
          <div className="monitor-head"><span>SYMBOL</span><span>PRICE</span><span>TODAY</span><span>VOLUME</span><span>SCORE</span></div>
          {candidates.slice(0,18).map(c=>{ const direction=movementClass(c.change_pct); return <div className={`monitor-row ${direction}`} key={c.symbol}><b>{c.symbol}</b><span className="monitor-price">{c.last_price?money(c.last_price):'—'}</span><span className={direction}>{c.change_pct==null?'—':`${c.change_pct >= 0 ? '+' : ''}${pct(c.change_pct)}`}</span><span>{c.volume?Number(c.volume).toLocaleString():'—'}</span><span>{Number(c.score||0).toFixed(1)}</span></div> })}
        </div>
      </section>
      ...
      <div className="portfolio-controls-stack">
        <section className="panel portfolio">
          ...
          <div className="stat-grid"><Stat label="CASH" v={money(portfolio.cash)}/><Stat label="BUYING POWER" v={money(portfolio.buying_power)}/></div>
          <div className="subhead">OPEN POSITIONS</div>
          <div className="panel-scroll positions-list">
            {(portfolio.positions||[]).map(p=><div className="pos-row" key={p.symbol}><b>{p.symbol}</b><span>{Number(p.qty||0).toLocaleString()}</span><span>{money(p.market_value)}</span><span className={p.unrealized_pl>=0?'up':'down'}>{money(p.unrealized_pl)}</span></div>)}
        ...
        <section className="panel controls">
          ...
          <div className="budget-card">
            ...
            <div className="budget-track"><i style={{width:`${quota.budget ? Math.max(0,Math.min(100,(quota.remaining/quota.budget)*100)) : 0}%`}} /></div>
          </div>
          <div className="control-stack">
            <button className={autopilotOn ? 'primary' : ''} onClick={()=>post('/api/control/autopilot',{enabled:!autopilotOn})}>{autopilotOn?'AUTOPILOT ON':'AUTOPILOT OFF'}</button>
            <button onClick={runManualAi} disabled={manualAi || Boolean(status.kill_switch)}>{manualAi?'RUNNING AI…':'RUN AI NOW'}</button>
            <button className="danger" onClick={()=>{setKillSwitchLatched(true); post('/api/control/kill-switch',{active:true})}} disabled={Boolean(status.kill_switch)}>{status.kill_switch?'KILL SWITCH ACTIVE':'KILL SWITCH'}</button>
          </div>
        </section>
      </div>
      ...
      <section className="panel squawk">
        ...
        <div className="panel-scroll squawk-log">
          {logs.map((l,i)=><div className={`squawk-line ${l.kind}`} key={i}><time>{l.time}</time><b>{l.agent}</b><span>{l.msg}</span></div>)}
      ...
      <section className="panel decisions">
        ...
        <div className="panel-scroll scroll-pane standalone-table">
          {decisions.length>0 && <div className="table-head decision-head"><span>TIME</span><span>DECISION</span><span>SYMBOL</span><span>THESIS</span></div>}
          {decisions.slice(0,18).map(d=>
            <div className="decision-row" key={d.id}>
              <TimeCell ts={d.timestamp} />
              <span className={`decision-action ${(d.action||'').toLowerCase()}`}>{d.action||'—'}</span>
              <span>{d.symbol||'—'}</span>
              <button type="button" className="thesis-btn" onClick={()=>setActiveDecision(d)} aria-haspopup="dialog" aria-label={`View thesis for ${d.symbol||'this decision'}`}>
      ...
      <section className="panel broker-orders">
        ...
        <div className="panel-scroll scroll-pane standalone-table">
          {orders.length>0 && <div className="table-head order-head"><span>TIME</span><span>SYMBOL</span><span>SIDE</span><span>QTY</span><span>STATUS</span></div>}
          {orders.slice(0,18).map((o,i)=><div className="order-row" key={o.id||i}><TimeCell ts={o.timestamp} /><b>{o.symbol||'—'}</b><span>{o.action||'—'}</span><span>{Number(o.qty||0).toLocaleString()}</span><em>{o.status||'—'}</em></div>)}
...
function Metric({label,value,className=''}){return <div className="metric"><span>{label}</span><b className={className}>{value}</b></div>}
function Stat({label,v,cls=''}){return <div className="stat"><span>{label}</span><b className={cls}>{v}</b></div>}
function PanelTitle({children,aside=''}){return <div className="panel-title"><span>{children}</span><div className="panel-title-actions">{aside&&<em>{aside}</em>}<i/></div></div>}
...
function DecisionModal({decision,onClose}){
  ...
  return <div className="modal-overlay" onClick={onClose}>
    <div className="modal-card decision-modal" role="dialog" aria-modal="true" aria-labelledby="decision-modal-symbol" onClick={e=>e.stopPropagation()}>
      ...
      <div className="decision-modal-header">
        ...
        <span className={`decision-status-pill ${d.approved?'ok':'bad'}`}>{d.approved?'APPROVED':'REJECTED'}</span>
      </div>
      <div className="decision-modal-meta">
        <div><span>WHEN</span>{dt ? <><b>{fmtDateFull(dt)}</b><small>{fmtTimeFull(dt)}</small></> : <b>—</b>}</div>
