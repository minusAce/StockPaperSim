...
def build_features(bars: list[Any], daily_dollar_volume: float | None = None) -> dict[str, Any]:
    if not bars:
        return {"data_quality": "INSUFFICIENT", "last_price": None, "daily_dollar_volume": 0.0, "dollar_volume": 0.0, "minute_dollar_volume": 0.0}
    rows = []
    for b in bars:
        rows.append({"ts": getattr(b, "timestamp", None), "open": float(b.open), "high": float(b.high), "low": float(b.low), "close": float(b.close), "volume": float(b.volume)})
    ...
    return {
        ...
        "bars": len(df), "last_price": last, "return_1m": ret_1 * 100, "return_5m": ret_5 * 100, "return_20m": ret_20 * 100,
        ...
        "atr14": atr14, "atr_pct": (atr14 / last * 100) if last else 0.0, "annualized_volatility": vol_annualized, "rsi14": rsi,
