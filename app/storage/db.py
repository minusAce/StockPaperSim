...
from sqlalchemy import Boolean, Column, DateTime, Float, Integer, MetaData, String, Table, Text, case, create_engine, func, select
...
class Database:
    def __init__(self, settings: Settings):
        ...
        self.engine: Engine = create_engine(settings.database_url, future=True, pool_pre_ping=True, connect_args=connect_args)
    ...
    def _seed_control_state(self) -> None:
        ...
        with self._lock, self.engine.begin() as conn:
            ...
            missing = [{"key": key, "value": "1" if value else "0"} for key, value in defaults.items() if key not in rows]
            if missing:
                conn.execute(self.control_state.insert(), missing)
            if self.settings.reset_runtime_controls_on_start:
                for key, value in defaults.items():
                    conn.execute(self.control_state.update().where(self.control_state.c.key == key).values(value="1" if value else "0"))
    ...
    def agent_run(self, agent: str, symbol: str | None, model: str, ok: bool, input_data: Any, output_data: Any, error: str | None = None) -> None:
        with self._lock, self.engine.begin() as conn:
            conn.execute(self.agent_runs.insert().values(timestamp=utc_now(), agent=agent, symbol=symbol, model=model, ok=ok, input_json=self._json(input_data), output_json=self._json(output_data), error=error))
    ...
    def _decision_values(self, data: dict[str, Any]) -> dict[str, Any]:
        return {
            "id": data["id"], "timestamp": _parse_dt(data.get("timestamp")), "symbol": data["symbol"], "action": data["action"],
    ...
    def order(self, data: dict[str, Any]) -> None:
        values = {
            "id": data["id"], "timestamp": _parse_dt(data.get("timestamp")), "symbol": data["symbol"], "action": data["action"],
            "qty": float(data.get("qty", 0)), "status": data.get("status"), "alpaca_order_id": data.get("alpaca_order_id"),
    ...
    def update_order_by_alpaca_id(self, alpaca_order_id: str, status: str, payload: dict[str, Any]) -> None:
        with self._lock, self.engine.begin() as conn:
            conn.execute(self.orders.update().where(self.orders.c.alpaca_order_id == alpaca_order_id).values(status=status, payload_json=self._json(payload)))

    def event(self, event_type: str, payload: Any) -> None:
        with self._lock, self.engine.begin() as conn:
            conn.execute(self.events.insert().values(timestamp=utc_now(), event_type=event_type, payload_json=self._json(payload)))

    def portfolio_snapshot(self, payload: Any) -> None:
        with self._lock, self.engine.begin() as conn:
            conn.execute(self.portfolio_snapshots.insert().values(timestamp=utc_now(), payload_json=self._json(payload)))

    def journal(self, symbol: str | None, event: str, message: str, payload: Any = None) -> None:
        with self._lock, self.engine.begin() as conn:
            conn.execute(self.trade_journal.insert().values(timestamp=utc_now(), symbol=symbol, event=event, message=message, payload_json=self._json(payload or {})))

    def record_session_entry(self) -> int:
        """Increment today's successful BUY count used only as a soft PM activity objective."""
        today = utc_now().date().isoformat()
        with self._lock, self.engine.begin() as conn:
            day_row = conn.execute(select(self.control_state.c.value).where(self.control_state.c.key == "session_entry_day")).first()
            count_row = conn.execute(select(self.control_state.c.value).where(self.control_state.c.key == "session_entry_count")).first()
    ...
    def latest(self, table: str, limit: int = 50) -> list[dict[str, Any]]:
        mapping = {"agent_runs": self.agent_runs, "decisions": self.decisions, "orders": self.orders, "events": self.events, "portfolio_snapshots": self.portfolio_snapshots, "trade_journal": self.trade_journal}
    ...
    def counts(self) -> dict[str, int]:
        ...
        with self._lock, self.engine.connect() as conn:
            return {name: int(conn.execute(select(func.count()).select_from(table)).scalar_one()) for name, table in names.items()}

    def performance(self) -> dict[str, Any]:
        agent_stats: list[dict[str, Any]] = []
        with self._lock, self.engine.connect() as conn:
            rows = conn.execute(select(self.agent_runs.c.agent, func.count(self.agent_runs.c.id).label("runs"), func.sum(case((self.agent_runs.c.ok.is_(True), 1), else_=0)).label("ok_runs")).group_by(self.agent_runs.c.agent)).fetchall()
            for r in rows:
                agent_stats.append({"agent": r.agent, "runs": int(r.runs or 0), "successful": int(r.ok_runs or 0), "success_rate": (float(r.ok_runs or 0) / float(r.runs or 1))})
            decision_count = int(conn.execute(select(func.count()).select_from(self.decisions)).scalar_one())
            approved = int(conn.execute(select(func.count()).select_from(self.decisions).where(self.decisions.c.approved.is_(True))).scalar_one())
            orders = int(conn.execute(select(func.count()).select_from(self.orders)).scalar_one())
            filled = int(conn.execute(select(func.count()).select_from(self.orders).where(self.orders.c.status.in_(["filled", "partially_filled"]))).scalar_one())
        return {"agents": agent_stats, "decisions": decision_count, "approved_decisions": approved, "orders": orders, "filled_orders": filled}
