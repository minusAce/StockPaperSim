...
class TradingEngine:
    def __init__(self, settings: Settings, alpaca: AlpacaService, team: AgentTeam, risk: RiskEngine, execution: ExecutionEngine, db: Database, bus: EventBus):
        ...
        self._task = None; self._cycle_lock = asyncio.Lock(); self._event_trigger_task = None; self._analysis_task = None
        self.last_scan = 0.0; self.last_cycle = 0.0
        self._last_portfolio_refresh = 0.0; self._last_benchmark_refresh = 0.0; self._last_market_monitor_refresh = 0.0
    ...
    async def start(self):
        ...
        try: await self.alpaca.load_assets()
        except Exception as exc: logger.warning("Asset universe load failed: %s", exc)
        if self.autopilot:
        ...
        await self._publish("system", {"level": "INFO", "message": f"Trading floor online — paper mode — autopilot={'ON' if self.autopilot else 'OFF'}"})
    ...
    async def stop(self):
        ...
        if self._display_refresh_task:
            ...
            try: await self._display_refresh_task
            except asyncio.CancelledError: pass
        ...
        while self.running:
            try:
                ...
                if now - self.last_scan >= self.settings.scanner_interval_seconds:
                    await self.scan(); self.last_scan = now
    ...
    async def _display_refresh_loop(self) -> None:
        ...
        while self.running:
            try:
                await asyncio.sleep(2)
                if not self.autopilot or self.risk.kill_switch:
                    continue
                now = time.time()
    ...
    async def set_autopilot(self, enabled: bool) -> None:
        ...
        await self._publish("system", {"level": "INFO", "message": f"Autopilot {state} — automated API/AI activity {activity}"})
    ...
    def _snapshot_metrics(snapshot: dict[str, Any]) -> dict[str, float | None]:
        ...
        previous = snapshot.get("prev_daily_bar") or snapshot.get("previous_daily_bar") or snapshot.get("prevDailyBar") or {}
    ...
    async def scan(self):
        try:
            ...
            await self._publish("system", {"level": "WARN", "message": f"S&P 500 universe refresh unavailable; using Alpaca discovery overlay: {exc}"})
            sp500 = []
        try:
            ...
            await self._publish("system", {"level": "ERROR", "message": f"Scanner failed: {exc}"}); return
        ...
        ranked.sort(key=lambda x: (float(x.get("score") or 0), abs(float(x.get("change_pct") or 0)), float(x.get("daily_dollar_volume") or 0)), reverse=True)
        ...
        if self.settings.alpaca_data_feed == "iex":
            ...
            if self.settings.stream_symbol_limit > 30:
                logger.warning("STREAM_SYMBOL_LIMIT=%s exceeds Alpaca Basic IEX limit; clamping live subscriptions to 30", self.settings.stream_symbol_limit)
    ...
    async def analysis_cycle(self, trigger: str = "scheduled", execute_trades: bool = True):
        ...
        async with self._cycle_lock:
            if (not self.autopilot and not trigger.startswith("manual-test")) or self.risk.manual_pause or not self.candidates: return
            ...
            started = time.time(); self.cycle_count += 1
            self.db.journal(None, "CYCLE_START", f"Analysis cycle #{self.cycle_count} started with {cycle_candidates} research slots", {"trigger": trigger, "candidate_slots": cycle_candidates, "quota": quota})
            await self._publish(
                "system",
                {
                    "level": "INFO",
            ...
            if not eligible:
                await self._publish("system", {"level": "WARN", "message": "Analysis cycle skipped — no liquidity-qualified candidates available."})
            ...
            try:
                ...
                await self._publish("system", {"level": "ERROR", "message": f"OpenRouter daily AI quota reached; stopping cycle: {exc}"})
                self.last_cycle = time.time()
                return
            except Exception as exc:
                # Fail closed: fall back to the deterministic scanner ordering.
                # This keeps the cycle alive without inventing AI output.
                await self._publish("system", {"level": "WARN", "message": f"SCOUT unavailable; using deterministic scanner fallback: {exc}"})
                selected = []
            if not selected:
                selected = sorted(scout_payload, key=lambda x: float(x.get("score") or 0), reverse=True)[:cycle_candidates]
            try:
                ...
                await self._publish("system", {"level": "WARN", "message": f"MACRO unavailable; rejecting cycle for safety: {exc}"})
                self.last_cycle = time.time()
                return
            results = []
            for c in selected:
                ...
                try:
                    ...
                    for agent_name, report in result["reports"].items(): await self._publish("agent", {"agent": agent_name, "data": report})
                except OpenRouterQuotaExceeded as exc:
                    await self._publish("system", {"level": "ERROR", "message": f"OpenRouter daily AI quota reached; stopping cycle: {exc}"})
                    self.last_cycle = time.time()
                    return
                except Exception as exc:
                    await self._publish("system", {"level": "ERROR", "message": f"Research failed for {symbol}; candidate rejected: {exc}"})
            if not results:
                await self._publish("system", {"level": "WARN", "message": "No complete research packets available; cycle rejected before PM."})
                self.last_cycle = time.time()
                return
            portfolio = await self._portfolio_payload()
            activity = self.db.session_entry_status()
            activity.update({"target_entries": self.settings.activity_target_entries, "remaining_target": max(0, self.settings.activity_target_entries - int(activity.get("entries_today", 0) or 0))})
            pm_payload = {"portfolio": portfolio, "macro": macro_data, "research": results, "activity_objective": activity, "risk_limits": {"max_position_weight": self.settings.max_position_weight, "max_order_notional": self.settings.max_order_notional, "min_confidence": self.settings.min_confidence, "min_liquidity_dollars": self.settings.min_liquidity_dollars}}
            try:
                ...
                await self._publish("system", {"level": "WARN", "message": f"PM unavailable; fail-safe NO_TRADE: {exc}"})
                self.last_cycle = time.time()
                return
            proposals = [p.model_dump() for p in pm.proposals if p.action != "HOLD"]
            if pm.environment != "TRADE" or not proposals:
                self.last_cycle = time.time(); return
            ...
            if duplicates or outside_research:
                ...
                await self._publish("system", {"level": "WARN", "message": f"PM output rejected for safety — {'; '.join(reasons)}"})
                self.last_cycle = time.time()
                return
            try:
                ...
                await self._publish("system", {"level": "WARN", "message": f"LLM RISK unavailable; rejecting cycle for safety: {exc}"})
                self.last_cycle = time.time()
                return
            open_orders = await self.alpaca.open_orders()
            for proposal in proposals:
                symbol = proposal["symbol"].upper(); research = next((r for r in results if r["symbol"] == symbol), None)
                if not research: continue
                f = research["features"]; price = _num(f.get("last_price")) or 0.0; daily_dollar_volume = _num(f.get("daily_dollar_volume", f.get("dollar_volume"))) or 0.0
                position = portfolio.get("positions_by_symbol", {}).get(symbol, {}); equity = float(portfolio.get("equity", 0) or 0); current_weight = float(position.get("weight", 0) or 0)
                if proposal["action"] == "BUY":
                    desired = max(0.0, float(proposal["target_weight"]) - current_weight) * equity
                    qty = desired / price if price else 0.0
                else:
                    qty = min(float(position.get("qty", 0) or 0) * 0.25, float(position.get("qty", 0) or 0))
                enriched = {**proposal, "symbol": symbol, "price": price, "quantity": qty, "daily_dollar_volume": daily_dollar_volume, "dollar_volume": daily_dollar_volume}
                llm_risk = reviews.get(symbol, {})
                if llm_risk.get("llm_risk") == "REJECT": approved, reason, meta = False, "LLM_RISK_REJECT", {}
                else: approved, reason, meta = self.risk.validate(enriched, portfolio, len(open_orders))
                qty = float(meta.get("quantity", qty)) if approved else qty
                decision = {"id": str(uuid.uuid4()), "symbol": symbol, "action": proposal["action"], "confidence": proposal["confidence"], "thesis": proposal["thesis"], "approved": approved, "rejection_reason": None if approved else reason, "quantity": qty, "price": price, "trigger": trigger}
                self.db.decision(decision); self.db.journal(symbol, "DECISION", f"{proposal['action']} {'APPROVED' if approved else 'REJECTED'} — {reason}", decision)
                ...
                self.db.journal(symbol, "ORDER", f"{result.action} {result.qty:.4f} submitted: {result.status}", result.model_dump())
                await self._publish("trade", result.model_dump())
            self.last_cycle = time.time()
            duration = time.time() - started
            self.db.journal(None, "CYCLE_COMPLETE", f"Analysis cycle #{self.cycle_count} complete in {duration:.1f}s", {"trigger": trigger, "duration_seconds": duration, "quota": self.team.quota_status(), "activity": self.db.session_entry_status()})
    ...
    async def _refresh_portfolio(self):
        ...
        equity = float(account.get("equity") or 0); cash = float(account.get("cash") or 0); buying_power = float(account.get("buying_power") or 0); last_equity = float(account.get("last_equity") or equity)
        daily_pnl = equity - last_equity; daily_pnl_pct = daily_pnl / last_equity if last_equity else 0
        by_symbol = {}; clean = []
        for p in positions:
            symbol = str(p.get("symbol", "")).upper(); mv = float(p.get("market_value") or 0); qty = float(p.get("qty") or 0); pl = float(p.get("unrealized_pl") or 0)
            item = {"symbol": symbol, "qty": qty, "market_value": mv, "weight": abs(mv) / equity if equity else 0, "unrealized_pl": pl, "current_price": float(p.get("current_price") or 0), "avg_entry_price": float(p.get("avg_entry_price") or 0), "unrealized_plpc": float(p.get("unrealized_plpc") or 0), "change_today": float(p.get("change_today") or 0)}
            by_symbol[symbol] = item; clean.append(item)
        self.portfolio = {"equity": equity, "cash": cash, "buying_power": buying_power, "daily_pnl": daily_pnl, "daily_pnl_pct": daily_pnl_pct, "positions": clean, "positions_by_symbol": by_symbol, "updated_at": time.time()}
        self.db.portfolio_snapshot(self.portfolio); await self._publish("portfolio", self.portfolio)

    async def _portfolio_payload(self):
        await self._refresh_portfolio(); return self.portfolio

    async def _on_market_event(self, kind: str, payload: dict[str, Any]):
        self.market_event_count += 1; self.last_market_event = payload
        if not self.autopilot:
            return
        if kind == "bar":
            previous = self.alpaca.previous_bar.get(payload["symbol"], {}); move = ((payload["close"] / previous["close"] - 1) * 100) if previous.get("close") else 0.0
            await self._publish("market", {**payload, "move_pct": move})
            if abs(move) >= self.settings.event_trigger_pct and self.autopilot and not self.risk.manual_pause:
                await self._trigger_event_cycle(payload["symbol"], "price_move")

    async def _trigger_event_cycle(self, symbol: str, reason: str):
    ...
    async def _on_trade_update(self, data: dict[str, Any]):
        ...
        self.db.event("trade_update", data); self.db.journal(symbol, event, f"Broker update: {status}", data)
    ...
    async def _publish(self, kind: str, data):
        await self.bus.publish(kind, data); self.db.event(kind, data)
