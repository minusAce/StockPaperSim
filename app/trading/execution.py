...
class ExecutionEngine:
    ...
    async def execute(self, symbol: str, action: str, qty: float) -> ExecutionResult:
        ...
        try:
            ...
            result = ExecutionResult(ok=False, symbol=symbol, action=action, qty=qty, order_id=None, status="ERROR",
                                     error=str(exc))
            self.db.order({"id": execution_id, "symbol": symbol, "action": action, "qty": qty, "status": "ERROR",
